...
# ╔═╡ f9f18e5b-69b9-4ce4-8e4a-d2804850dd92
xrange = [-10, 10]

# ╔═╡ a3dcd5d5-d4f9-40a0-9aa9-81b3b444b83d
yrange = [-10, 10]

# ╔═╡ 955318fc-56d0-49af-980e-8f6926e19868
target = [5,4]

# ╔═╡ 162b587c-9466-434b-b319-45d5379bd567
radius = 3

# ╔═╡ 5c8205c2-f1de-484a-9736-472ade05d3ae
initialstate = Product([Uniform(xrange...), Uniform(yrange...)])

# ╔═╡ 59462d4a-41e5-417b-8104-d642643aad6e
function circle(xy, r)
	θ = LinRange(0, 2π, 500)
	return xy[1] .+ r*sin.(θ), xy[2] .+ r*cos.(θ)
end

# ╔═╡ 7330bcda-a664-443d-b537-4dcebef164d0
θ(h) = Normal(h, 0.5)

# ╔═╡ 85a494e0-7b75-43b3-aadd-84db85dfccc0
D = Normal(1, 0.1)

# ╔═╡ db48172f-6b27-4890-9057-7d8b38b674ba
function generate_trajectory(; s=[0,0], h=90, d=20)
	τ = []
	h = rand(θ(h))
	for t in 1:d
		push!(τ, s)
		d = rand(D)
		h = rand(θ(h))
		s = [s[1] + d*sin(h), s[2] + d*cos(h)]
	end
	return τ
end

# ╔═╡ 0e80d295-2f97-47b3-8437-64603285178b
τ = generate_trajectory()

# ╔═╡ 2e354f0b-99c4-4fd0-9ccb-b2a0465c6d48
num_traj = 50

# ╔═╡ 7d01b08b-f063-4c8e-b6ea-00fbd910405e
begin
	Random.seed!(0)
	trajs = [generate_trajectory(s=rand(initialstate), h=rand(-90:90)) for _ in 1:num_traj]
end
...
# ╔═╡ 7bae976e-5d9f-4cb1-b9a5-531ceb4d6cc5
function plot_trajectories(trajs, target, radius, ranges; t=missing)
	plot()
	for τ in trajs
		if ismissing(t)
			t = length(τ)
	...
	plot!(circle(target, radius), seriestype=[:shape], lw=0.5,
		c=:yellow, linecolor=:black, label=false,
		fillalpha=0.2)
	plot!([target[1], target[1]-radius], [target[2], target[2]],
		c=:gray, label=false)
	scatter!([target[1]], [target[2]], c=:yellow, label=false)
...
plot_trajectories(trajs, target, radius, [xrange, yrange])

# ╔═╡ b76d1fc7-2048-4c22-9783-394236dc9e91
function find_violations(trajs, target, radius)
	violated_trajs = []
	for τ in trajs
		for t in eachindex(τ)
			if norm(τ[t] - target) < radius
...
violated_trajs = find_violations(trajs, target, radius)

# ╔═╡ 567b5027-1c84-4eb1-93f6-ca86fd215d0c
plot_trajectories(violated_trajs, target, radius, [xrange, yrange])

# ╔═╡ 6d9a19d0-592c-4dac-bf85-0d66d925f46c
@bind t Slider(1:length(violated_trajs[1]))

# ╔═╡ 02d9bab2-a3c3-465e-94ea-4818d9b751a5
plot_trajectories(violated_trajs, target, radius, [xrange, yrange]; t)

# ╔═╡ 0551a9aa-b9bc-4014-acd6-7829895c610d
function create_gif(trajs, target, radius, ranges)
	frames = Frames(MIME("image/png"), fps=2)
	for t in 1:length(trajs[1])
		frame = plot_trajectories(trajs, target, radius, ranges; t)
		push!(frames, frame)
	end
...
create_gif(violated_trajs, target, radius, [xrange, yrange])

# ╔═╡ 639dcdfd-0fbf-4f1a-b807-782c2f8196d3
md"""
# Debug
"""

# ╔═╡ afd2ce48-d97e-44a2-8f68-7cdc9779636a
# begin
# 	import Pkg
# 	pkg"add https://github.com/UW-ACL/SCPToolbox.jl"
# end

# ╔═╡ ced729ca-7690-498b-9e0d-36f332ee9f25
# module Quadrotor
...
# ╔═╡ 60a89ecb-1187-42f7-a5c8-37cc3e4cb777
# using SCPToolbox

# ╔═╡ 4b6e92fa-7a7f-461e-a618-a28b3fe73926
md"""
Y. Wan et al., "A Smooth-Turn Mobility Model for Airborne Networks". _IEEE Transactions on Vehicular Technology_, 2013.
"""

# ╔═╡ 5d5a0121-1ced-4372-b2e6-c4f2e63acdf1
begin
	const V = 1
	const R = 0.2
	aₜ(t) = 0
...
# ╔═╡ 94844033-a423-41b7-977f-f8aeebb9ae7a
# begin
# 	plot([1,2,3,4,5,6], [1,4,3,5,4,6], seriestype=:cubic_spline)
# 	scatter!([1,2,3,4,5,6], [1,4,3,5,4,6])
# end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "162274dd85bbe8b6905f983f396bcbaab87a149d"
